...
contract LendingInvariantHandler is Test {
    ...
    mapping(uint256 => bool) public hasStandingOffer;
    mapping(uint256 => uint32) public lastOfferRate;
    mapping(uint256 => uint32) public lastRefiOfferRate;
    ...
    function offerBorrow(uint256 loanSeed, bool allowAnyLiquidator, uint32 rateSeed) external {
        ...
        // V2 requires a strictly improving (lower) rate when a previous offer exists.
        uint32 prevRate = lastOfferRate[lendingId];
        uint32 ceiling = hasStandingOffer[lendingId] && prevRate > 1 ? prevRate - 1 : BASE_RATE * 2;
        uint32 rate = uint32(bound(uint256(rateSeed), 1, uint256(ceiling)));

        // Borrower address used as caller is rotated from rateSeed to keep some lender variety.
        address caller = (rateSeed & 1) == 0 ? lender1 : lender2;

        vm.startPrank(caller);
        try lending.offerBorrow(lendingId, uint128(loan.amountDemanded), rate, allowAnyLiquidator) {
            hasStandingOffer[lendingId] = true;
            lastOfferRate[lendingId] = rate;
        } catch {}
        vm.stopPrank();
    }
    ...
    function acceptBorrow(uint256 loanSeed) external {
        if (lendingIds.length == 0) return;
        uint256 lendingId = lendingIds[loanSeed % lendingIds.length];
        if (!hasStandingOffer[lendingId]) return;

        vm.startPrank(borrower);
        try lending.acceptOffer(lendingId) {
            hasStandingOffer[lendingId] = false;
        } catch {}
        vm.stopPrank();
    }
    ...
    function offerRefi(uint256 loanSeed, uint32 rateSeed) external {
        ...
        // V2 requires a strictly improving (lower) rate when a previous refi offer exists at the current nonce.
        openLending.RefiLendingOffer memory existing = lending.getRefiLendingOffer(lendingId);
        uint32 ceiling = (existing.lender != address(0) && existing.rate > 1) ? existing.rate - 1 : uint32(2e8);
        uint32 rate = uint32(bound(uint256(rateSeed), 1, uint256(ceiling)));
        ...
        ) returns (uint256 refiNonce) {
            lastRefiNonce[lendingId] = refiNonce;
            lastRefiOfferRate[lendingId] = rate;
        } catch {}
        vm.stopPrank();
    }
    ...
    function acceptRefi(uint256 loanSeed) external {
        ...
        if (refiNonce == 0) return;

        vm.startPrank(borrower);
        try lending.acceptRefiOffer(lendingId, refiNonce) {
            acceptedRefiNonce[lendingId] = refiNonce;
            lastRefiOfferRate[lendingId] = 0;
            openLending.LendingView memory loan = lending.getLending(lendingId);
            openLending.RefiParams memory refiParams = lending.getRefiParams(lendingId);
...
contract LendingInvariantsTest is StdInvariant, Test {
    ...
    function invariant_currentRefiNonceRemainsUnacceptedAndAcceptedNonceSticks() public view {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            openLending.LendingView memory loan = lending.getLending(lendingId);
            assertFalse(
                lending.getRefiNonceClosed(lendingId, loan.refiOfferNonce),
                "current refi nonce should not already be marked closed"
            );

            uint256 acceptedNonce = handler.acceptedRefiNonce(lendingId);
            if (acceptedNonce != 0) {
                assertTrue(
                    lending.getRefiNonceClosed(lendingId, acceptedNonce),
                    "accepted refi nonce should remain marked closed"
                );
