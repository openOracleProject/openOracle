...
contract HelperCoverageTest is Test {
    ...
    function testRefiViewHelpers_ReturnStoredState() public {
        ...
        openLending.RefiLendingOffer memory refiOffer = lending.getRefiLendingOffer(lendingId);
        ...
        assertFalse(lending.getRefiNonceClosed(lendingId, refiNonce), "refi nonce should be open before accept");

        vm.prank(borrower);
        lending.acceptRefiOffer(lendingId, refiNonce);

        assertTrue(lending.getRefiNonceClosed(lendingId, refiNonce), "refi nonce should be closed after accept");
    }
    ...
    function _setupCustomLoan(
        ...
        vm.prank(lender);
        lending.offerBorrow(lendingId, borrowAmount, INTEREST_RATE, allowAnyLiquidator);
