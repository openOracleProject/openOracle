...
contract GasCompensationTest is OpenLendingBaseTest {
    ...
    function testUnderwaterLiquidationWithStagedRefi_RefundsGasComp() public {
        ...
        vm.prank(liquidator);
        lending.liquidate{value: 1e15}(lendingId, 6 ether * 1e18 / 10 ether, type(uint128).max, paramHash, 0, 1e15);
    ...
    function testUnsuccessfulLiq_DoesNotRefundGasComp() public {
        ...
        bytes32 paramHash = lending.getParamHash(lendingId);
        vm.prank(liquidator);
        lending.liquidate{value: 1e15}(lendingId, 12 ether * 1e18 / 10 ether, type(uint128).max, paramHash, 0, 1e15);
        uint256 reportId = oracle.nextReportId() - 1;
    ...
    function testMultiCycle_EthAccountingClean() public {
        ...
        // Refi #1, then cancel
        vm.prank(borrower);
        lending.refinance{value: secondComp}(lendingId, 0, 0, 0, secondComp, _standardInterestRateParams(), _zeroOracleParams(), bytes32(0), 0, type(uint128).max);
