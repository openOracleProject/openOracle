...
contract AmortizationTest is OpenLendingBaseTest {
    ...
    function testLiqDebt_PrincipalAmortReducesLiquidationExposure() public {
        ...
        bytes32 paramHash = lending.getParamHash(lendingId);
        vm.prank(liquidator);
        lending.liquidate{value: 1e15}(lendingId, _priceRatioFor(6 ether), type(uint128).max, paramHash, 0, 1e15);
        ...
        openLend.LendingArrangement memory loanFinal = lending.getLending(lendingId);
        assertFalse(loanFinal.finished, "amortized principal kept the position above the threshold");
    }
    ...
    /// @dev Refi acceptance resets interestAccrued, interestPaid, lastTouch, and recomputes commitmentInterest.
    ...
    /// @dev getParamHash zeros principal, interestAccrued, interestPaid, lastTouch — so a stale hash
    ///      stays valid after partial repayments that change those fields.
    function testHash_StaleHashRemainsValidAfterAmortMutations() public {
        ...
        vm.prank(borrower);
        lending.repayDebt(lendingId, 1 ether, staleHash, 0, type(uint128).max);
    }
    ...
    function testHash_ExpectedMaxPrincipalPinsAmortizedValue() public {
        ...
        vm.prank(borrower);
        lending.repayDebt(lendingId, 1 wei, bytes32(0), 0, newPrincipal);
    }
    ...
    function testHash_ProjectedFailedLiqHashAcceptedByLend() public {
        ...
        uint256 reportId = _liquidateAt(liquidator, lendingId, _priceRatioFor(12 ether), 1e15);
    ...
    function testLiq_FractionZero_LenderKeepsBuffer() public {
        ...
        uint256 liquidatorBefore = supplyToken.balanceOf(liquidator);
        ...
        int256 liquidatorNet = int256(supplyToken.balanceOf(liquidator)) - int256(liquidatorBefore);
    ...
    /// @dev liquidate() reverts unless msg.value equals the settlerReward arg.
    function testSettlerReward_MustEqualMsgValue() public {
        ...
        bytes32 paramHash = lending.getParamHash(lendingId);
        vm.prank(liquidator);
        vm.expectRevert(abi.encodeWithSelector(openLend.InvalidInput.selector, "msg.value != settlerReward"));
        lending.liquidate{value: 1e15}(lendingId, _priceRatioFor(8 ether), type(uint128).max, paramHash, 0, 2e15);
    }

    /// @dev Custom settlerReward is forwarded to the settler when an external party calls oracle.settle().
    function testSettlerReward_CustomAmountForwardedToSettler() public {
        uint256 lendingId = _setupActiveLoan(0);
        vm.warp(block.timestamp + 10 days);

        uint96 customReward = 5e15;
        uint256 reportId = _liquidateAt(liquidator, lendingId, _priceRatioFor(12 ether), customReward);

        uint256 settlerBefore = settler.balance;
        vm.warp(block.timestamp + ORACLE_SETTLEMENT_TIME + 1);
        vm.prank(settler);
        oracle.settle(reportId);

        assertEq(settler.balance - settlerBefore, customReward, "settler received the custom reward");
    }
    ...
    function testRegression_RecoverFullStakeToLiquidatorAndAmortTouched() public {
        ...
        vm.mockCallRevert(address(lending), abi.encodeWithSelector(openLend.onSettle.selector), "callback bricked");
        vm.prank(settler);
        oracle.settle(reportId);
        vm.clearMockedCalls();
    ...
    function testRegression_FuzzPartialRepaysReconcile(uint128 a1, uint128 a2, uint128 a3) public {
        ...
        vm.warp(block.timestamp + 1 days);
        vm.prank(borrower);
        lending.repayDebt(lendingId, a1, bytes32(0), 0, type(uint128).max);
        vm.warp(block.timestamp + 1 days);
        vm.prank(borrower);
        lending.repayDebt(lendingId, a2, bytes32(0), 0, type(uint128).max);
        vm.warp(block.timestamp + 1 days);
        vm.prank(borrower);
