...
contract OpenOraclePackedRoundTripTest is BaseGGTest {
    ...
    function testRoundTrip_ReportSubmitted_DecodesAllFields() public {
        ...
        assertEq(
            oracle.settlementEligibility(reportId),
            uint256(expected.reportTimestamp) + expected.settlementTime,
            "stored settlement eligibility"
        );
    ...
    function testRoundTrip_ReportDisputed_DecodesAllFields() public {
        ...
        uint128 newAmount1;
        unchecked {
            newAmount1 = uint128(uint256(preGame.currentAmount1) * preGame.multiplier / 100);
        }
        ...
        vm.recordLogs();
        vm.prank(bob);
        oracle.dispute(reportId, newAmount1, newAmount2, bob, false, false, preGame, helper, _emptyTiming());
        Vm.Log[] memory logs = vm.getRecordedLogs();
        ...
        assertEq(
            oracle.settlementEligibility(reportId),
            uint256(expected.reportTimestamp) + expected.settlementTime,
            "dispute-updated settlement eligibility"
        );
