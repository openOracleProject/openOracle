...
contract RemainingEdgeCoverageTest is OpenLendingBaseTest {
    ...
    function testGrabOracleGameFeesAny_SweepsBorrowFeesAndSecondSweepIsNoOp() public {
        ...
        _disputeAndSwap(reportId, address(borrowToken), 20 ether, 20 ether, disputer, 8 ether, stateHash);

        uint256 feeAccrued = IOpenOracle2(address(oracle)).tokenHolder(feeRecipient, address(borrowToken));
        assertGt(feeAccrued, 0, "borrow-side fee should accrue");
        ...
        uint256 borrowerInternalBefore = IOpenOracle2(address(oracle)).tokenHolder(borrower, address(borrowToken));
        ...
        assertEq(borrowToken.balanceOf(borrower), borrowerBefore, "borrower wallet unchanged");
        assertEq(borrowToken.balanceOf(lender), lenderBefore, "lender wallet unchanged");
        assertEq(borrowToken.balanceOf(liquidator), liquidatorBefore, "liquidator wallet unchanged");

        uint256 borrowerPiece = swept / 2;
        uint256 lenderPiece = borrowerPiece / 2;
        uint256 liquidatorPiece = swept - borrowerPiece - lenderPiece;
        assertEq(
            IOpenOracle2(address(oracle)).tokenHolder(borrower, address(borrowToken)) - borrowerInternalBefore,
            borrowerPiece + (borrowerInternalBefore == 0 ? 1 : 0),
            "borrower internal fee piece"
        );
        assertEq(
            IOpenOracle2(address(oracle)).tokenHolder(lender, address(borrowToken)) - lenderInternalBefore,
            lenderPiece + (lenderInternalBefore == 0 ? 1 : 0),
            "lender internal fee piece"
        );
        assertEq(
            IOpenOracle2(address(oracle)).tokenHolder(liquidator, address(borrowToken)) - liquidatorInternalBefore,
            liquidatorPiece + (liquidatorInternalBefore == 0 ? 1 : 0),
        ...
        borrowerInternalBefore = IOpenOracle2(address(oracle)).tokenHolder(borrower, address(borrowToken));
        ...
        assertEq(IOpenOracle2(address(oracle)).tokenHolder(borrower, address(borrowToken)), borrowerInternalBefore, "second sweep should be a no-op for borrower internal");
        assertEq(IOpenOracle2(address(oracle)).tokenHolder(lender, address(borrowToken)), lenderInternalBefore, "second sweep should be a no-op for lender internal");
        assertEq(IOpenOracle2(address(oracle)).tokenHolder(liquidator, address(borrowToken)), liquidatorInternalBefore, "second sweep should be a no-op for liquidator internal");
    }
    ...
    function testFuzz_RequestBorrowInvalidOracleParamsRollsBackState(uint8 badSeed) public {
        openLend.OracleParams memory bad;
        uint8 which = badSeed % 10;
        if (which == 0) bad = _badOracle_settlementTimeBelowMin();
        else if (which == 1) bad = _badOracle_settlementTimeAboveMax();
        else if (which == 2) bad = _badOracle_disputeDelayGtSettlement();
        else if (which == 3) bad = _badOracle_escFactorBelowMin();
        else if (which == 4) bad = _badOracle_escFactorAboveMax();
        else if (which == 5) bad = _badOracle_initLiquidityBelowMin();
        else if (which == 6) bad = _badOracle_initLiquidityAboveMax();
        else if (which == 7) bad = _badOracle_escFactorBelowInitLiquidity();
        else if (which == 8) bad = _badOracle_feeTooHigh();
        else bad = _badOracle_multiplierAboveMax();

        uint256 nextBefore = lending.nextLendingId();
        ...
        assertEq(lending.nextLendingId(), nextBefore, "nextLendingId unchanged");
        assertEq(supplyToken.balanceOf(borrower), borrowerSupplyBefore, "borrower supply unchanged");
        assertEq(supplyToken.balanceOf(address(lending)), lendingSupplyBefore, "lending supply unchanged");
        assertEq(borrower.balance, borrowerEthBefore, "borrower ETH unchanged");
        assertEq(address(lending).balance, lendingEthBefore, "lending ETH unchanged");
    }
