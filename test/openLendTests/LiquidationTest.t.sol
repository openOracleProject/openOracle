...
contract LiquidationTest is OpenLendingBaseTest {
    ...
    function testLiquidation_SuccessWithEquityRemaining() public {
        ...
        _disputeAndSwap(reportId, address(supplyToken), 20 ether, 12 ether, disputer1, 8 ether, stateHash);

        vm.warp(block.timestamp + ORACLE_DISPUTE_DELAY + 1);
        _disputeAndSwap(reportId, address(supplyToken), 40 ether, 32 ether, disputer2, 12 ether, stateHash);
    ...
    function testLiquidation_SuccessNoEquityRemaining() public {
        ...
        vm.warp(block.timestamp + ORACLE_DISPUTE_DELAY + 1);
        _disputeAndSwap(reportId, address(supplyToken), 20 ether, 10 ether, disputer1, 6 ether, stateHash);
    ...
    function testLiquidation_FailsPriceDoesntLiquidate() public {
        ...
        vm.warp(block.timestamp + ORACLE_DISPUTE_DELAY + 1);
        _disputeAndSwap(reportId, address(supplyToken), 20 ether, 30 ether, disputer1, 12 ether, stateHash);
    ...
    function testLiquidation_MaliciousEligibilityPriceStillForfeitsStakeIfOracleSettlesHealthy() public {
        uint256 lendingId = _setupLoan(5e6);
        vm.warp(block.timestamp + 10 days);
        ...
        // This low amount2 makes the submitted report liquidation-eligible at entry.
        _liquidate(liquidator, lendingId, 6 ether);

        uint256 reportId = oracle.nextReportId() - 1;
        IOpenOracle2.OracleGame memory initialGame = IOpenOracle2(address(oracle)).storedGame(reportId);
        ...
        vm.warp(block.timestamp + ORACLE_DISPUTE_DELAY + 1);
        _disputeAndSwap(reportId, address(supplyToken), 20 ether, 30 ether, disputer1, 12 ether, bytes32(0));
        ...
        assertEq(
            ...
            liquidatorSupplyBefore - initialLiquidity - tokenStake,
            "liquidator funded initial liquidity and forfeited stake"
        );
        assertEq(
            borrowToken.balanceOf(liquidator),
            liquidatorBorrowBefore - 6 ether,
            "liquidator funded malicious amount2"
        );
    }
    ...
    function testGracePeriod_BorrowerCanRepayDuringGrace() public {
        ...
        uint256 reportId = oracle.nextReportId() - 1;
        _disputeToNonLiquidatingPrice(reportId, address(supplyToken), disputer1);
        ...
        openLend.LendingArrangement memory loan = lendView.getLending(lendingId);
        // settle delta = ORACLE_SETTLEMENT_TIME + 1 = 301; gracePeriod = 1800 + 301
        assertEq(loan.gracePeriod, 1800 + (ORACLE_DISPUTE_DELAY + ORACLE_SETTLEMENT_TIME), "exact gracePeriod from near-maturity failed liq");
        ...
        vm.prank(borrower);
        lending.repayDebt(lendingId, paymentAmount, bytes32(0), 0, type(uint128).max, false);
    ...
    function testGracePeriod_BorrowerCannotTopUpDuringGrace() public {
        uint256 lendingId = _setupLoan(5e6);
        ...
        vm.prank(borrower);
        vm.expectRevert(LendErrors.InGracePeriod.selector);
    ...
    /// @dev `liquidate` requires the current loose paramHash.
    function testLiquidate_AcceptsCorrectParamHash() public {
        uint256 lendingId = _setupLoan(5e6);
        vm.warp(block.timestamp + 10 days);
        ...
        assertTrue(lendView.getLending(lendingId).inLiquidation,
            "liquidate succeeds with current paramHash");
    }
    ...
    ///      the old clone still holds (and can still distribute) any fees it accrued.
    function testMultipleFeeReceivers_OldOneStillSweepable() public {
        ...
        assertEq(oracleFeeReceiver(feeRecipient1).lendingId(), lendingId, "receiver1 immutable lendingId");
        assertEq(oracleFeeReceiver(feeRecipient2).lendingId(), lendingId2, "receiver2 immutable lendingId");
    ...
    function testFeeReceivers_RemainReportBoundAcrossRefiAndLaterLiquidation() public {
        ...
        vm.startPrank(disputer2);
        lending.lend(lendingId,lendView.getParamHash(lendingId), 0, type(uint128).max, 0, 0, 5e6, disputer2, address(0));
        vm.stopPrank();
        ...
        // A different lendingId cannot resolve beneficiaries for either receiver by reusing old reportIds.
