...
contract CommitmentExtremesTest is OpenLendingBaseTest {
    ...
    function testCommitmentZero_MidTermCostsLessThanCommitmentFull() public {
        ...
        vm.prank(borrower);
        lending.repayDebt(flexLoan, type(uint128).max, bytes32(0), 0, type(uint128).max, false);
        uint256 flexCost = borrowerBefore - borrowToken.balanceOf(borrower);

        borrowerBefore = borrowToken.balanceOf(borrower);
        vm.prank(borrower);
        lending.repayDebt(fixedLoan, type(uint128).max, bytes32(0), 0, type(uint128).max, false);
    ...
    function testLiquidation_IdenticalAcrossExtremes() public {
        ...
        // Liquidate flex loan
        bytes32 paramHash = lendView.getLiquidateParamHash(flexLoan);
        vm.prank(liquidator);
        ...
        _disputeAndSwap(flexReportId, address(supplyToken), 20 ether, 10 ether, disputer, 6 ether, stateHash);
        vm.warp(block.timestamp + 301);
        vm.prank(settler);
        _settleOracle(flexReportId);
        ...
        paramHash = lendView.getLiquidateParamHash(fixedLoan);
        vm.prank(liquidator);
        ...
        (,,, uint48 reportTs,,,) = _reportStatus(fixedReportId);
        vm.warp(uint256(reportTs) + 61);
        _disputeAndSwap(fixedReportId, address(supplyToken), 20 ether, 10 ether, disputer, 6 ether, stateHash);
        (,,, uint48 disputeTs,,,) = _reportStatus(fixedReportId);
        vm.warp(uint256(disputeTs) + 301);
        vm.prank(settler);
        _settleOracle(fixedReportId);
    ...
    function testCommitment_ParamHashDistinguishesExtremes() public {
        ...
        bytes32 flexHash = lendView.getLiquidateParamHash(flexLoan);
        bytes32 fixedHash = lendView.getLiquidateParamHash(fixedLoan);
