...
contract WithdrawingReporterCallback {
    ...
    function submit(uint256 reportId, uint128 amount1, uint128 amount2, bytes32 sh) external {
        ...
        oracle.submitInitialReport(reportId, amount1, amount2, sh, false, false, empty);
    }
...
contract OpenOracleGGStorageModeTest is BaseGGTest {
    ...
    function testEscalationHaltCapBehavior_StorageMode() public {
        ...
        vm.prank(alice);
        oracle.submitInitialReport(reportId, 10e18, 10e18, stateHash, false, false, _emptyTiming());

        // 10 * 2 = 20 > 15, so cap at 15.
        vm.prank(bob);
        oracle.disputeAndSwap(reportId, address(token1), 15e18, 12e18, 10e18, stateHash, false, false, _emptyTiming());
    ...
    function testEscalationHaltCapBehavior_RevertsWrongAmount() public {
        ...
        vm.prank(alice);
        oracle.submitInitialReport(reportId, 5e18, 5e18, stateHash, false, false, _emptyTiming());

        // expected = 5 * 1.5 = 7.5e18; bob sends 7.6e18 -> should revert.
        vm.prank(bob);
        vm.expectRevert(OpenOracleErrors.InvalidAmount1.selector);
        oracle.disputeAndSwap(reportId, address(token1), 76e17, 6e18, 5e18, stateHash, false, false, _emptyTiming());

        // Correct amount succeeds.
        vm.prank(bob);
        oracle.disputeAndSwap(reportId, address(token1), 75e17, 6e18, 5e18, stateHash, false, false, _emptyTiming());
        ...
        vm.prank(charlie);
        oracle.disputeAndSwap(reportId, address(token1), 8e18, 7e18, 6e18, stateHash, false, false, _emptyTiming());
    }
    ...
    function testInvalidStateHash_Reverts_StorageMode() public {
        ...
        oracle.submitInitialReport(reportId, 1e18, 2000e18, wrong, false, false, _emptyTiming());
    }
    ...
    function testInvalidAmount1_Reverts_StorageMode() public {
        ...
        vm.prank(bob);
        vm.expectRevert(OpenOracleErrors.InvalidAmount1.selector);
        oracle.submitInitialReport(reportId, 2e18, 2000e18, stateHash, false, false, _emptyTiming());
    }
    ...
    function testInvalidAmount2_Reverts_StorageMode() public {
        ...
        oracle.submitInitialReport(reportId, 1e18, 0, stateHash, false, false, _emptyTiming());
    }
