...
contract InvariantHandler {
    ...
    function submitInitial(uint256 idSeed) external {
        ...
        try oracle.submitInitialReport(reportId, 1e18, 1e18, sh, false, false, _emptyTiming()) {} catch {}
    }
    ...
    function dispute(uint256 idSeed) external {
        ...
        try oracle.disputeAndSwap(
            reportId, address(token1), uint128(nextA1), uint128(newA2), oldA2, sh, false, false, _emptyTiming()
        ) {} catch {}
    }
