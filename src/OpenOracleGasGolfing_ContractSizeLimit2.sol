...
contract OpenOracle is OpenOracleErrors {
    ...
    struct OracleGame {
        bytes32 stateHash; //slot
        uint128 currentAmount1;
        uint128 currentAmount2; //slot
        address payable currentReporter;
        uint48 reportTimestamp;
        uint48 settlementTimestamp;  //slot
        address payable initialReporter;
        uint48 lastReportOppoTime; // status
        // some space here

        uint128 exactToken1Report;
        uint128 escalationHalt;  //slot
        uint96 fee;
        uint96 settlerReward; //slot
        address token1;
        uint48 settlementTime; //slot
        address token2;
        bool timeType;
        uint24 feePercentage; //slot
        uint24 protocolFee;
        uint16 multiplier;
        uint24 disputeDelay; // meta
        address callbackContract; //slot
        uint32 numReports;
        uint32 callbackGasLimit;
        bytes4 callbackSelector;
        address protocolFeeRecipient; //slot
        bool trackDisputes; // extra data //slot
    }
    ...
    event ReportInstanceCreatedCalldata(uint256 indexed reportId);
    event InitialReportSubmittedCalldata(uint256 indexed reportId);
    event ReportDisputedCalldata(uint256 indexed reportId);
    event ReportSettledCalldata(uint256 indexed reportId);
        ...
        returns (uint256 reportId)
    {
        ...
        if (isStorageMode) {
            ...
            emit ReportInstanceCreatedCalldata(reportId);
        }
    ...
    function submitInitialReportCalldata(
        ...
        if (preimageHelper.isStorageMode) revert InvalidMode();
        _submitInitialReport(
            reportId,
            amount1,
            amount2,
    ...
    function _submitInitialReport(
        ...
        // can go through wrong function deliberately
        // 1. go through calldata path when it's actually storage mode
        //     - hash shouldnt be able to match because the helper in createReportInstance contained storage mode true
        // 2. go through storage path when mode is actually calldata
        //     - exactToken1Report will be 0. This can't happen for storage mode - you can't initialize a 0 exactToken1Report in that mode
        //
        // What if you pass a reportId thats too high? either exactToken1Report == 0, or the hash doesn't match (that reportid cannot be in an onchain hash without being created)
        // meaning we can remove 'if (reportId >= nextReportId) revert InvalidReportId();'

        if (isStorageMode) {
            oracle = oracleGame[reportId];
            if (oracle.exactToken1Report == 0) revert InvalidMode();
        } else {
            oracle = params;
        ...
        if (isStorageMode) {
            ...
            emit InitialReportSubmittedCalldata(reportId);
        }
    }
    ...
    function _disputeAndSwap(
        ...
        if (isStorageMode) {
            ...
            emit ReportDisputedCalldata(reportId);
        }
    }
    ...
    function settle(uint256 reportId) external {
        OracleGame memory empty;
        PreimageHelper memory empty2;
        _settle(reportId, empty, empty2, true);
    }

    function settleCalldata(uint256 reportId, OracleGame calldata oracleGameData, PreimageHelper calldata helper)
        external
    {
        if (helper.isStorageMode) revert InvalidMode();
        _settle(reportId, oracleGameData, helper, false);
    }
        ...
        internal
    {
        ...
        // what about passing reportId too high here? same impact: cannot breach the storage mode checks
        if (isStorageMode) {
            oracle = oracleGame[reportId];
            if (oracle.exactToken1Report == 0) revert InvalidMode();
        } else {
        ...
        if (isStorageMode) {
            ...
            emit ReportSettledCalldata(reportId);
        }
    }
    ...
    function _creditEth(address recipient, uint256 amount) internal {
        uint256 bal = ethHolder[recipient];
        ethHolder[recipient] = bal == 0 ? amount + 1 : bal + amount;
    }
