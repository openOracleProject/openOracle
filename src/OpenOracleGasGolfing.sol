...
contract OpenOracle is OpenOracleErrors {
    ...
    mapping(bytes32 => bool) public dustedPair;
        ...
        returns (uint256 reportId)
    {
        ...
        if (params.feePercentage > 0) oracle.feePercentage = params.feePercentage;
        ...
        if (params.disputeDelay > 0) oracle.disputeDelay = params.disputeDelay;
        if (params.protocolFee > 0) oracle.protocolFee = params.protocolFee;
        ...
        if (params.trackDisputes) oracle.trackDisputes = params.trackDisputes;
        oracle.callbackGasLimit = params.callbackGasLimit;
        if (params.protocolFeeRecipient != address(0)) oracle.protocolFeeRecipient = params.protocolFeeRecipient;
     ...
     * @dev Funds are returned to reporter when settled or disputed. Internal balance spends preserve the virtual 1-unit sentinel.
     */
     ...
     * @dev Funds are returned to disputer when settled or disputed. Internal balance spends preserve the virtual 1-unit sentinel.
     */
