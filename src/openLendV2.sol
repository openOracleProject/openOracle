...
contract openLending is ReentrancyGuard {
    ...
    struct LendingArrangement {
        ...
        RefiLendingOffer refiLendingOffer;
    ...
    event OfferAccepted(uint256 lendingId);
    ...
    function cancelBorrowRequest(uint256 lendingId) external nonReentrant {
        LendingArrangement storage lending = lendingArrangements[lendingId];
        LendingOffer storage offer = lending.lendingOffer;
        address offeror = offer.lender;
        ...
        if (offeror != address(0)) {
            uint128 amount = offer.amount;
            delete lending.lendingOffer;
            _transferTokens(lending.borrowToken, address(this), offeror, amount);
        }
        ...
        nonReentrant
    {
        ...
        uint32 prevRate = offer.rate;
        address prevLender = offer.lender;
        uint128 prevAmount = offer.amount;
        bool prevLenderExists = (prevLender != address(0));
        ...
        if (rate == 0) revert InvalidInput("rate cannot be 0");
        if (prevLenderExists && rate >= prevRate) revert InvalidInput("must improve rate");
        if (owedAtMaturity > type(uint128).max) revert InvalidInput("owedAtMaturity too high");

        offer.lender = msg.sender;
        offer.amount = amount;
        offer.rate = rate;
        offer.allowAnyLiquidator = allowAnyLiquidator;
        offer.offerTime = currentTime;

        IERC20(borrowToken).safeTransferFrom(msg.sender, address(this), amount);

        if (prevLenderExists) {
            _transferTokens(borrowToken, address(this), prevLender, prevAmount);
        }
    ...
    function offerRefiBorrow(
    ...
    ) external nonReentrant returns (uint256 refiNonce) {
        LendingArrangement storage lending = lendingArrangements[lendingId];
        refiNonce = lending.refiOfferNonce;
        RefiLendingOffer storage refi = lending.refiLendingOffer;
        ...
        uint32 prevRate = refi.rate;
        address prevLender = refi.lender;
        uint128 prevAmount = refi.amount;
        bool prevLenderExists = (prevLender != address(0));
        ...
        emit RefiBorrowOffered(msg.sender, lendingId, rate, refiNonce);
     ...
     * @param lendingId Unique identification number of lending instance
     */
    function cancelBorrowOffer(uint256 lendingId) external nonReentrant {
        LendingArrangement storage lending = lendingArrangements[lendingId];
        LendingOffer storage offer = lending.lendingOffer;
        ...
        if (lending.active) revert InvalidInput("lendingId active");
        if (lender == address(0) || amount == 0) revert InvalidInput("no borrow offer");
     ...
     */
    function cancelRefiBorrowOffer(uint256 lendingId, uint256 refiNonce)
        external
        nonReentrant
    {
        LendingArrangement storage lending = lendingArrangements[lendingId];
        ...
        if (lender == address(0) || amount == 0) revert InvalidInput("no refi offer");
        if (msg.sender != lender) revert InvalidInput("msg.sender");
        if (block.timestamp < refiOfferTime + 60) revert InvalidInput("cancel too soon");

        delete lending.refiLendingOffer;
     ...
     * @param lendingId Unique identification number of lending instance
     */
    function acceptOffer(uint256 lendingId) external nonReentrant {
        LendingArrangement storage lending = lendingArrangements[lendingId];
        LendingOffer storage offer = lending.lendingOffer;
        ...
        uint128 amount = offer.amount;
        if (offer.lender == address(0) || amount == 0) revert InvalidInput("no offer");
        address borrowToken = lending.borrowToken;
     ...
     * @param refiNonce Unique identification number of refinancing round (one per new loan)
     */
    function acceptRefiOffer(uint256 lendingId, uint256 refiNonce) external nonReentrant {
        LendingArrangement storage lending = lendingArrangements[lendingId];
        RefiLendingOffer storage refi = lending.refiLendingOffer;
        ...
        uint256 extraDemanded = lending.refiParams.extraDemanded;
        uint128 supplyPulled = lending.refiParams.supplyPulled;
        ...
        if (refiNonce != lending.refiOfferNonce) revert InvalidInput("invalid refi nonce");
        if (currentTime >= lending.start + term + lending.gracePeriod) revert InvalidInput("expired");
        if (refi.lender == address(0) || refi.amount == 0) revert InvalidInput("no refi offer");
    ...
    function repayDebt(uint256 lendingId, uint128 amount) external nonReentrant {
        ...
        address borrowToken = lending.borrowToken;
        address refiOfferor = refi.lender;
        uint128 refiAmount = refi.amount;
        ...
        if (amount >= netTerminalDebt) {
            ...
            if (refiOfferor != address(0)) {
                delete lending.refiLendingOffer;
                _transferTokens(borrowToken, address(this), refiOfferor, refiAmount);
            }
            ...
            emit DebtRepaid(lendingId, netTerminalDebt);
        } else {
        ...
        nonReentrant
    {
        ...
        if (liqThresh < borrowValueInSupplyTerms) {
            ...
            if (offeror != address(0)) {
                uint128 amount = refi.amount;
                delete lending.refiLendingOffer;
                _transferTokens(borrowToken, address(this), offeror, amount);
            }
    ...
    function getLendingOffer(uint256 lendingId) external view returns (LendingOffer memory) {
        return lendingArrangements[lendingId].lendingOffer;
    }

    function getRefiLendingOffer(uint256 lendingId)
        external
        view
        returns (RefiLendingOffer memory)
    {
        return lendingArrangements[lendingId].refiLendingOffer;
    }

    function getRefiNonceClosed(uint256 lendingId, uint256 refiNonce) external view returns (bool) {
        LendingArrangement storage lending = lendingArrangements[lendingId];
